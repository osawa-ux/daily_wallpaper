...
[MIT License](LICENSE)。本リポジトリの **コード** は MIT で自由に利用・改変・再配布できます。

`quotes.json` に含まれる名言テキストは各著作者のものであり、引用範囲での利用を想定しています。商用利用や大規模再配布の際は、各 quote の権利状況を個別にご確認ください。

`assets/backgrounds/` および `assets/fonts/` は空でリポジトリに含まれます。各自で配置するフォント・画像のライセンスはご自身でご確認ください（リポジトリには第三者素材を同梱していません）。
